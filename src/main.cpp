...
#if defined(NDEBUG)
#error "Jericoin cannot be compiled without assertions."
#endif
...
bool ContextualCheckZerocoinSpend(const CTransaction& tx, const libzerocoin::CoinSpend* spend, CBlockIndex* pindex, const uint256& hashBlock)
{
    ...
    if (IsSerialInBlockchain(spend->getCoinSerialNumber(), nHeightTx))
        return error("%s : zJHL spend with serial %s is already in block %d\n", __func__,
...
{
    //Check to see if the zJHL is properly signed
    if (pindex->nHeight >= Params().Zerocoin_Block_V2_Start()) {
        try {
            if (!spend->HasValidSignature())
                return error("%s: V2 zJHL spend does not have a valid signature\n", __func__);
        ...
        if (spend->getSpendType() != expectedType) {
            return error("%s: trying to spend zJHL without the correct spend type. txid=%s\n", __func__,
                         tx.GetHash().GetHex());
        }
    }
    ...
    if (!spend->HasValidSerial(Params().Zerocoin_Params(fUseV1Params)))  {
        ...
        if (!isBlockBetweenFakeSerialAttackRange(pindex->nHeight))
            return error("%s : zJHL spend with serial %s from tx %s is not in valid range\n", __func__,
...
bool CheckZerocoinSpend(const CTransaction& tx, bool fVerifySignature, CValidationState& state, bool fFakeSerialAttack)
{
    ...
    for (const CTxIn& txin : tx.vin) {
        ...
        if (isPublicSpend) {
            ...
            if (!ZJHLModule::parseCoinSpend(txin, tx, prevOut, publicSpend)){
        ...
        if (isPublicSpend) {
            ...
            if (!ZJHLModule::validateInput(txin, prevOut, tx, ret)){
...

    {
        ...
        if (hasZcSpendInputs) {
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("%s : zJHL spend tx %s already in block %d",
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (isPublicSpend) {
                    libzerocoin::ZerocoinParams* params = Params().Zerocoin_Params(false);
                    PublicCoinSpend publicSpend(params);
                    if (!ZJHLModule::ParseZerocoinPublicSpend(txIn, tx, state, publicSpend)){
                        return false;
                    }
            ...
            // Check that zJHL mints (if included) are not already known
...
int64_t GetBlockValue(int nHeight)
{
    ...
    int64_t nSubsidy = 0;
    if (nHeight > 1900000)   	return 10 * COIN;
    if (nHeight > 1600000)   	return 60 * COIN;
    if (nHeight > 1500000)   	return 120 * COIN;
    if (nHeight > 1400000)   	return 240 * COIN;
    if (nHeight > 1300000)   	return 420 * COIN;
    if (nHeight > 1200000)   	return 550 * COIN;
    if (nHeight > 1100000)   	return 500 * COIN;
    if (nHeight > 1000000)   	return 390 * COIN;
    if (nHeight >  950000)   	return 370 * COIN;
    if (nHeight >  900000)   	return 350 * COIN;
    if (nHeight >  850000)   	return 300 * COIN;
    if (nHeight >  800000)   	return 200 * COIN;
    if (nHeight >  750000)   	return 140 * COIN;
    if (nHeight >  700000)   	return 120 * COIN;
    if (nHeight >  650000)   	return 100 * COIN;
    if (nHeight >  600000)   	return 90 * COIN;
    if (nHeight >  550000)   	return 80 * COIN;
    if (nHeight >  500000)   	return 70 * COIN;
    if (nHeight >  400000)	return 60 * COIN;
    if (nHeight >  320800)	return 50 * COIN;
    if (nHeight >  210100)	return 40 * COIN;
    if (nHeight >  100200)	return 30 * COIN;
    if (nHeight >   50000)	return 20 * COIN;
    if (nHeight >    5000)	return 10 * COIN;
    if (nHeight >       1)	return 7 * COIN;
    if (nHeight !=      1)	return 7 * COIN;
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount, bool isZJHLStake)
{
        ...
        for (const CTxIn& in : tx.vin) {
            ...
            if (in.IsZerocoinSpend() || isPublicSpend) {
                ...
                if (isPublicSpend) {
                    ...
                    if (!ZJHLModule::ParseZerocoinPublicSpend(in, tx, state, publicSpend)){
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from Jericoin
...
{
    RenameThread("jericoin-scriptch");
...
void RecalculateZJHLMinted()
{
    CBlockIndex *pindex = chainActive[Params().Zerocoin_StartHeight()];
    uiInterface.ShowProgress(_("Recalculating minted ZJHL..."), 0);
    while (true) {
        ...
        if (pindex->nHeight % 1000 == 0) {
            ...
            uiInterface.ShowProgress(_("Recalculating minted ZJHL..."), percent);
        }
...
void RecalculateZJHLSpent()
{
    CBlockIndex* pindex = chainActive[Params().Zerocoin_StartHeight()];
    uiInterface.ShowProgress(_("Recalculating spent ZJHL..."), 0);
    while (true) {
        if (pindex->nHeight % 1000 == 0) {
            LogPrintf("%s : block %d...\n", __func__, pindex->nHeight);
            int percent = std::max(1, std::min(99, (int)((double)(pindex->nHeight - Params().Zerocoin_StartHeight()) * 100 / (chainActive.Height() - Params().Zerocoin_StartHeight()))));
            uiInterface.ShowProgress(_("Recalculating spent ZJHL..."), percent);
        }
        ...
        //Remove spends from zJHL supply
...
bool RecalculateJHLSupply(int nHeightStart)
{
    ...
    uiInterface.ShowProgress(_("Recalculating JHL supply..."), 0);
    while (true) {
        if (pindex->nHeight % 1000 == 0) {
            LogPrintf("%s : block %d...\n", __func__, pindex->nHeight);
            int percent = std::max(1, std::min(99, (int)((double)((pindex->nHeight - nHeightStart) * 100) / (chainActive.Height() - nHeightStart))));
            uiInterface.ShowProgress(_("Recalculating JHL supply..."), percent);
        }
...
{
    // Jericoin: recalculate Accumulator Checkpoints that failed to database properly
...
bool UpdateZJHLSupply(const CBlock& block, CBlockIndex* pindex, bool fJustCheck)
{
    std::list<CZerocoinMint> listMints;
    ...
    // A one-time event where only the zJHL supply was off (due to serial duplication off-chain on main net)
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    for (unsigned int i = 0; i < block.vtx.size(); i++) {
        ...
        if (tx.HasZerocoinSpendInputs()) {
            ...
            // Check that zJHL mints are not already known
            if (tx.HasZerocoinMintOutputs()) {
                for (auto& out : tx.vout) {
                    if (!out.IsZerocoinMint())
                        continue;
    ...
    if (pindex->nHeight == Params().Zerocoin_Block_RecalculateAccumulators() + 1) {
        RecalculateZJHLMinted();
        RecalculateZJHLSpent();
        RecalculateJHLSupply(Params().Zerocoin_StartHeight());
    }

    //Track zJHL money supply in the block index
    if (!UpdateZJHLSupply(block, pindex, fJustCheck))
        return state.DoS(100, error("%s: Failed to calculate new zJHL supply for block=%s height=%d", __func__,
...
bool CheckColdStakeFreeOutput(const CTransaction& tx, const int nHeight)
{
    ...
    if (outs >=3 && lastOut.scriptPubKey != tx.vout[outs-2].scriptPubKey) {
        ...
        // cold staking is active much after nPublicZCSpends so GetMasternodePayment is always 3 JHL.
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zJHL spends in this block
        if (tx.HasZerocoinSpendInputs()) {
            for (const CTxIn& txIn : tx.vin) {
                bool isPublicSpend = txIn.IsZerocoinPublicSpend();
                if (txIn.IsZerocoinSpend() || isPublicSpend) {
                    ...
                    if (std::count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zJHL serial %s in block\n Block: %s",
...
bool CheckWork(const CBlock block, CBlockIndex* const pindexPrev)
{
    ...
    if (block.nBits != nBitsRequired) {
        // Jericoin Specific reference to the block with the wrong threshold was used.
        if ((block.nTime == (uint32_t) Params().JericoinBadBlockTime()) && (block.nBits == (uint32_t) Params().JericoinBadBlocknBits())) {
            // accept Jericoin block minted with incorrect proof of work threshold
            return true;
        }
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (isPoS) {
        ...
        std::vector<CTxIn> zJHLInputs;

        for (const CTxIn& stakeIn : stakeTxIn.vin) {
            if(stakeIn.IsZerocoinSpend()){
                zJHLInputs.push_back(stakeIn);
            }else{
                pivInputs.push_back(stakeIn);
            }
        }
        const bool hasJHLInputs = !pivInputs.empty();
        const bool hasZJHLInputs = !zJHLInputs.empty();
        ...
        if (isBlockFromFork) {
            ...
            while (!chainActive.Contains(prev)) {
                ...
                for (const CTransaction& t : bl.vtx) {
                    ...
                    for (const CTxIn& in: t.vin) {
                        ...
                        if(hasZJHLInputs && in.IsZerocoinSpend())
            ...
            // Now that this loop if completed. Check if we have zJHL inputs.
            if(hasZJHLInputs) {
                for (const CTxIn& zPivInput : zJHLInputs) {
...
bool static ProcessMessage(CNode* pfrom, std::string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // Jericoin: We use certain sporks during IBD, so check to see if they are
