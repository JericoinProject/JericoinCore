...
// The zJHL block index is the first appearance of the accumulator checksum that was used in the spend
...
bool CZPivStake::GetModifier(uint64_t& nStakeModifier)
{
    ...
    // zJHL staking is disabled long before block v7 (and checkpoint is not included in blocks since v7)
...
{
    //The unique identifier for a zJHL is a hash of the serial
...
{
    //Create an output returning the zJHL that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZJHLOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zJHL output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zJHL", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZJHLOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zJHL output", __func__);
...
{
    CzJHLTracker* zpivTracker = pwallet->zpivTracker.get();
    CMintMeta meta;
...
{
    //The unique identifier for a JHL stake is the outpoint
