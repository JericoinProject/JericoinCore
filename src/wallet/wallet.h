...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 JHL at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zJHL send
enum ZerocoinSpendStatus {
    ZJHL_SPEND_OKAY = 0,                            // No error
    ZJHL_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZJHL_WALLET_LOCKED = 2,                         // Wallet was locked
    ZJHL_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZJHL_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZJHL_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZJHL_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZJHL_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZJHL_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZJHL_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZJHL_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZJHL_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZJHL_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZJHL_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZJHL_SPENT_USED_ZJHL = 14,                      // Coin has already been spend
    ZJHL_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZJHL_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    bool CreateZJHLOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    ...
    */
    ...
    bool fBackupMints;
    std::unique_ptr<CzJHLTracker> zpivTracker;
    ...
    void setZWallet(CzJHLWallet* zwallet);
    CzJHLWallet* getZWallet();
    ...
    /// Get 10000 JHL output and keys which can be used for the Masternode
    ...
    /** zJHL reset */
    boost::signals2::signal<void()> NotifyzJHLReset;
