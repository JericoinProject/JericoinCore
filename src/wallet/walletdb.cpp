...
bool CWalletDB::WriteZJHLSeed(const uint256& hashSeed, const std::vector<unsigned char>& seed)
{
...
bool CWalletDB::EraseZJHLSeed()
{
    uint256 hash;
    if(!ReadCurrentSeedHash(hash)){
        return error("Failed to read a current seed hash");
    }
    if(!WriteZJHLSeed(hash, ToByteVector(base_uint<256>(0) << 256))) {
...
bool CWalletDB::EraseZJHLSeed_deprecated()
{
    return Erase(std::string("dzs"));
}

bool CWalletDB::ReadZJHLSeed(const uint256& hashSeed, std::vector<unsigned char>& seed)
{
    return Read(std::make_pair(std::string("dzs"), hashSeed), seed);
}

bool CWalletDB::ReadZJHLSeed_deprecated(uint256& seed)
{
    return Read(std::string("dzs"), seed);
}

bool CWalletDB::WriteZJHLCount(const uint32_t& nCount)
{
    return Write(std::string("dzc"), nCount);
}

bool CWalletDB::ReadZJHLCount(uint32_t& nCount)
{
