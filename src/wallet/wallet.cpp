...
int CWallet::ScanForWalletTransactions(CBlockIndex* pindexStart, bool fUpdate, bool fromStartup)
{
    ...
    bool fCheckZJHL = GetBoolArg("-zapwallettxes", false);
    if (fCheckZJHL)
        zpivTracker->Init();
    ...
    CBlockIndex* pindex = pindexStart;
    {
        ...
        while (pindex) {
            ...
            if (fCheckZJHL && pindex->nHeight >= Params().Zerocoin_StartHeight()) {
...
{
    if (fMatureOnly) {
        // This code is not removed just for when we back to use zJHL in the future, for now it's useless,
...
bool CWallet::GetBudgetSystemCollateralTX(CWalletTx& tx, uint256 hash, bool useIX)
{
    ...
    vecSend.push_back(std::make_pair(scriptChange, BUDGET_FEE_TX_OLD)); // Old 50 JHL collateral
...
bool CWallet::GetBudgetFinalizationCollateralTX(CWalletTx& tx, uint256 hash, bool useIX)
{
    ...
    std::string strFail = "";
    std::vector<std::pair<CScript, CAmount> > vecSend;
    vecSend.push_back(std::make_pair(scriptChange, BUDGET_FEE_TX)); // New 5 JHL collateral
            ...
            while (true) {
                ...
                if (!SelectCoins(nTotalValue, setCoins, nValueIn, coinControl, coin_type, useIX, false, fIncludeDelegated)) {
                    if (coin_type == ALL_COINS) {
                        ...
                        strFailReason = _("Unable to locate enough funds for this transaction that are not equal 10000 JHL.");
                    } else if (coin_type == ONLY_NONDENOMINATED_NOT10000IFMN) {
                        strFailReason = _("Unable to locate enough Obfuscation non-denominated funds for this transaction that are not equal 10000 JHL.");
                    } else {
                ...
                if (nChange > 0) {
                    ...
                    // change transaction isn't always pay-to-jericoin-address
                    CScript scriptChange;
        ...
        )
{
    ...
    for (std::unique_ptr<CStakeInput>& stakeInput : listInputs) {
        ...
        //Masternode payment
        FillBlockPayee(txNew, nMinFee, true, stakeInput->IsZJHL());
...
void CWallet::CreateAutoMintTransaction(const CAmount& nMintAmount, CCoinControl* coinControl)
{
    if (nMintAmount > 0){
        ...
        LogPrintf("CWallet::AutoZeromint() @ block %ld: successfully minted %ld zJHL. Current percentage of zJHL: %lf%%\n",
...
void CWallet::AutoZeromint()
{
    ...
    // zJHL are integers > 0, so we can't mint 10% of 9 JHL
    if (nBalance < 10){
        LogPrint("zero", "CWallet::AutoZeromint(): available balance (%ld) too small for minting zJHL\n", nBalance);
        return;
    }

    // Percentage of zJHL we already have
    ...
    if(dPercentage >= nZeromintPercentage){
        LogPrint("zero", "CWallet::AutoZeromint() @block %ld: percentage of existing zJHL (%lf%%) already >= configured percentage (%d%%). No minting needed...\n",
                  chainActive.Tip()->nHeight, dPercentage, nZeromintPercentage);
        return;
    }

    // zJHL amount needed for the target percentage
    nToMintAmount = ((nZerocoinBalance + nBalance) * nZeromintPercentage / 100);

    // zJHL amount missing from target (must be minted)
    nToMintAmount = (nToMintAmount - nZerocoinBalance) / COIN;

    // Use the biggest denomination smaller than the needed zJHL We'll only mint exact denomination to make minting faster.
    ...
    // If a preferred denomination is used (means nPreferredDenom != 0) do nothing until we have enough JHL to mint this denomination
...
bool CWallet::CreateZJHLOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint)
{
    ...
    zwalletMain->GenerateDeterministicZJHL(denomination, coin, dMint);
...
bool CWallet::CreateZerocoinMintTransaction(const CAmount nValue, CMutableTransaction& txNew, std::vector<CDeterministicMint>& vDMints, CReserveKey* reservekey, int64_t& nFeeRet, std::string& strFailReason, const CCoinControl* coinControl, const bool isZCSpendChange)
{
    ...
    while (true) {
        ...
        if (!CreateZJHLOutPut(denomination, outMint, dMint)) {
    ...
    // Sign if these are jericoin outputs - NOTE that zJHL outputs are signed later in SoK
    if (!isZCSpendChange) {
        int nIn = 0;
        for (const std::pair<const CWalletTx*, unsigned int>& coin : setCoins) {
            if (!SignSignature(*this, *coin.first, txNew, nIn++)) {
...
bool CWallet::CheckCoinSpend(libzerocoin::CoinSpend& spend, libzerocoin::Accumulator& accumulator, CZerocoinSpendReceipt& receipt)
{
    if (!spend.Verify(accumulator)) {
        receipt.SetStatus(_("The transaction did not verify"), ZJHL_BAD_SERIALIZATION);
    ...
    if (Params().NetworkID() != CBaseChainParams::REGTEST && IsSerialKnown(spend.getCoinSerialNumber())) {
        //Tried to spend an already spent zJHL
        receipt.SetStatus(_("The coin spend has been used"), ZJHL_SPENT_USED_ZJHL);
                         ...
                         CZerocoinSpendReceipt& receipt, libzerocoin::SpendType spendType, CBlockIndex* pindexCheckpoint)
{
    ...
    receipt.SetStatus(_("Transaction Mint Started"), ZJHL_TXMINT_GENERAL);
    ...
    for (auto &it : mapMintsSelected) {
        ...
        if (!GenerateAccumulatorWitness(&coinWitness, mapAccumulators, pindexCheckpoint)) {
            receipt.SetStatus(_("Couldn't generate the accumulator witness"),
                              ZJHL_FAILED_ACCUMULATOR_INITIALIZATION);
        ...
        if (nVersion >= libzerocoin::PrivateCoin::PUBKEY_VERSION) {
            ...
            if (!mint.GetKeyPair(key))
                return error("%s: failed to set zJHL privkey mint version=%d", __func__, nVersion);
        ...
        try {
            ...
            if (!CheckCoinSpend(spend, accumulator, receipt)) {
                receipt.SetStatus(_("CoinSpend: failed check"), ZJHL_SPEND_ERROR);
                return error("%s : %s", __func__, receipt.GetStatusMessage());
            }
            ...
            receipt.SetStatus(_("CoinSpend: Accumulator witness does not verify"), ZJHL_INVALID_WITNESS);
            return error("%s : %s", __func__, receipt.GetStatusMessage());
        }
    }
    ...
    receipt.SetStatus(_("Spend Valid"), ZJHL_SPEND_OKAY); // Everything okay
                                    ...
                                    CZerocoinSpendReceipt& receipt, libzerocoin::SpendType spendType, CBlockIndex* pindexCheckpoint)
{
    ...
    for (auto &it : mapMintsSelected) {
        ...
        if (!GetTransaction(mint.GetTxHash(), txMint, hashBlock)) {
            receipt.SetStatus(strprintf(_("Unable to find transaction containing mint %s"), mint.GetTxHash().GetHex()), ZJHL_TXMINT_GENERAL);
            return false;
        } else if (mapBlockIndex.count(hashBlock) < 1) {
            // check that this mint made it into the blockchain
            receipt.SetStatus(_("Mint did not make it into blockchain"), ZJHL_TXMINT_GENERAL);
            return false;
        }
        ...
        if (outputIndex == -1) {
            receipt.SetStatus(_("Pubcoin not found in mint tx"), ZJHL_TXMINT_GENERAL);
            return false;
        }

        mint.SetOutputIndex(outputIndex);
        CTxIn in;
        if(!ZJHLModule::createInput(in, mint, hashTxOut, spendVersion)) {
            receipt.SetStatus(_("Cannot create public spend input"), ZJHL_TXMINT_GENERAL);
            return false;
        }
        vin.emplace_back(in);
        ...
        bool isPublicSpend)
{
    ...
    int nStatus = ZJHL_TRX_FUNDS_PROBLEMS;
    ...
    if (nValue < 1) {
        receipt.SetStatus(_("Value is below the smallest available denomination (= 1) of zJHL"), nStatus);
        return false;
    }

    // Create transaction
    nStatus = ZJHL_TRX_CREATE;
    ...
    const int nMaxSpends = Params().Zerocoin_MaxPublicSpendsPerTransaction(); // Maximum possible spends for one zJHL public spend transaction
    std::vector<CMintMeta> vMintsToFetch;
    if (vSelectedMints.empty()) {
        //  All of the zJHL used in the public coin spend are mature by default (everything is public now.. no need to wait for any accumulation)
        ...
        // Select the zJHL mints to use in this spend
    ...
    wtxNew.BindWallet(this);
    {
        LOCK2(cs_main, cs_wallet);
        {
            ...
            for (std::pair<CBitcoinAddress*,CAmount> pair : addressesTo){
                ...
                //add output to jericoin address to the transaction (the actual primary spend taking place)
                // TODO: check value?
            ...
            if (nBytes >= MAX_ZEROCOIN_TX_SIZE) {
                receipt.SetStatus(_("In rare cases, a spend with 7 coins exceeds our maximum allowable transaction size, please retry spend using 6 or less coins"), ZJHL_TX_TOO_LARGE);
                return false;
            }
    ...
    receipt.SetStatus(_("Transaction Created"), ZJHL_SPEND_OKAY); // Everything okay
...
)
{
    ...
    int nStatus = ZJHL_SPEND_ERROR;

    if (IsLocked()) {
        receipt.SetStatus("Error: Wallet locked, unable to create transaction!", ZJHL_WALLET_LOCKED);
        return false;
    }
    ...
    if (!CommitTransaction(wtxNew, reserveKey)) {
        ...
        for (CZerocoinSpend spend : receipt.GetSpends()) {
            if (!walletdb.EraseZerocoinSpendSerialEntry(spend.GetSerial())) {
                receipt.SetStatus("Error: It cannot delete coin serial number in wallet", ZJHL_ERASE_SPENDS_FAILED);
            }
        ...
        for (auto& dMint : vNewMints) {
            if (!walletdb.EraseDeterministicMint(dMint.GetPubcoinHash())) {
                receipt.SetStatus("Error: Unable to cannot delete zerocoin mint in wallet", ZJHL_ERASE_NEW_MINTS_FAILED);
            }
        }
    ...
    receipt.SetStatus("Spend Successful", ZJHL_SPEND_OKAY);  // When we reach this point spending zJHL was successful
...
void CWallet::setZWallet(CzJHLWallet* zwallet)
{
    zwalletMain = zwallet;
    zpivTracker = std::unique_ptr<CzJHLTracker>(new CzJHLTracker(strWalletFile));
}

CzJHLWallet* CWallet::getZWallet()
{
    return zwalletMain;
}
