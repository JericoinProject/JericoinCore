...
public:
    ...
    bool WriteZJHLSeed(const uint256& hashSeed, const std::vector<unsigned char>& seed);
    bool ReadZJHLSeed(const uint256& hashSeed, std::vector<unsigned char>& seed);
    bool ReadZJHLSeed_deprecated(uint256& seed);
    bool EraseZJHLSeed();
    bool EraseZJHLSeed_deprecated();

    bool WriteZJHLCount(const uint32_t& nCount);
    bool ReadZJHLCount(uint32_t& nCount);
