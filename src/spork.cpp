...
// Copyright (c) 2016-2019 The Jericoin developers
...
std::vector<CSporkDef> sporkDefs = {
    ...
    MAKE_SPORK_DEF(SPORK_5_MAX_VALUE,                       1000),          // 1000 JHL
    MAKE_SPORK_DEF(SPORK_8_MASTERNODE_PAYMENT_ENFORCEMENT,  4070908800ULL),          // active since the block 1800
...
// Jericoin: on startup load spork values from previous session if they exist in the sporkDB
...
void CSporkManager::ProcessSpork(CNode* pfrom, std::string& strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == "spork") {
        ...
        // Jericoin: add to spork database.
