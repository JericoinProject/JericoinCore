// Copyright (c) 2017-2019 The Jericoin developers
...
CzJHLWallet::CzJHLWallet(std::string strWalletFile)
{
    ...
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZJHLSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZJHLSeed_deprecated()) {
                    LogPrintf("%s: Updated zJHL seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
...
bool CzJHLWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZJHLCount(nCountLastUsed);
    else if (!walletdb.ReadZJHLCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzJHLWallet::Lock()
{
    seedMaster = 0;
}

void CzJHLWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzJHLWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZJHL(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzJHLWallet::LoadMintPoolFromDB()
{
...
void CzJHLWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzJHLWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzJHLWallet::SyncWithChain(bool fGenerateMintPool)
{
...
bool CzJHLWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZJHL(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
...
void CzJHLWallet::SeedToZJHL(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzJHLWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzJHLWallet::GenerateDeterministicZJHL(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzJHLWallet::GenerateMint(const uint32_t& nCount, const libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzJHLWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzJHLWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
