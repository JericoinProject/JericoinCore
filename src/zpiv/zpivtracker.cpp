// Copyright (c) 2018-2020 The Jericoin developers
...
CzJHLTracker::CzJHLTracker(std::string strWalletFile)
{
...
CzJHLTracker::~CzJHLTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzJHLTracker::Init()
{
...
bool CzJHLTracker::Archive(CMintMeta& meta)
{
...
bool CzJHLTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzJHLTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzJHLTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzJHLTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzJHLTracker::GetSerialHashes()
{
...
CAmount CzJHLTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzJHLTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzJHLTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzJHLTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzJHLTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzJHLTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzJHLTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzJHLTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzJHLTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzJHLTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzJHLTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzJHLWallet* zJHLWallet)
{
    bool iszJHLWalletInitialized = (NULL != zJHLWallet);
    CMintMeta meta;
    ...
    if (! iszJHLWalletInitialized)
        zJHLWallet = new CzJHLWallet(strWalletFile);
    meta.isSeedCorrect = zJHLWallet->CheckSeed(dMint);
    if (! iszJHLWalletInitialized)
        delete zJHLWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzJHLTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzJHLTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzJHLTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzJHLTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzJHLTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzJHLTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzJHLWallet* zJHLWallet = new CzJHLWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zJHLWallet);
        }
        delete zJHLWallet;
