...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
    ...
    foreach (const SendCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered jericoin address / amount:
...

    {
        ...
        if (recipients[0].useSwiftTX && total > sporkManager.GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 JHL.").arg(sporkManager.GetSporkValue(SPORK_5_MAX_VALUE)),
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction)
{
...

    {
        ...
        foreach (const SendCoinsRecipient& rcp, recipients) {
            ...
            } else if (!rcp.message.isEmpty()) // Message from normal jericoin:URI (jericoin:XyZ...?message=example)
            {
...
static void NotifyzJHLReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzJHLReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzJHLReset.connect(boost::bind(NotifyzJHLReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzJHLReset.disconnect(boost::bind(NotifyzJHLReset, this));
...
bool WalletModel::updateAddressBookPurpose(const QString &addressStr, const std::string& purpose) {
    ...
    if (address.IsStakingAddress())
        return error("Invalid Jericoin address, cold staking address");
    CKeyID keyID;
...
bool WalletModel::getKeyId(const CBitcoinAddress& address, CKeyID& keyID) {
    if (!address.IsValid())
        return error("Invalid Jericoin address");

    if (!address.GetKeyID(keyID))
        return error("Unable to get KeyID from Jericoin address");
