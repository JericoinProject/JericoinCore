...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset){
    if (!settings.contains("nDisplayUnit") || reset)
        settings.setValue("nDisplayUnit", BitcoinUnits::JHL);
    ...
    if (settings.contains("nAnonymizeJericoinAmount") || reset)
        SoftSetArg("-anonymizejericoinamount", settings.value("nAnonymizeJericoinAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in jericoin.cpp during shotdown to also remove the window positions
