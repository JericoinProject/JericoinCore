...
QList<TransactionRecord> TransactionRecord::decomposeTransaction(const CWallet* wallet, const CWalletTx& wtx)
{
    ...
    if (wtx.HasZerocoinSpendInputs()) {
        libzerocoin::CoinSpend zcspend = wtx.HasZerocoinPublicSpendInputs() ? ZJHLModule::parseCoinSpend(wtx.vin[0]) : TxInToZerocoinSpend(wtx.vin[0]);
    ...
    if (wtx.IsCoinStake()) {
        ...
        if (wtx.HasZerocoinSpendInputs() && (fZSpendFromMe || wallet->zpivTracker->HasMintTx(hash))) {
            ...
            sub.type = TransactionRecord::StakeZJHL;
...
bool IsZJHLType(TransactionRecord::Type type)
{
    switch (type) {
        case TransactionRecord::StakeZJHL:
...
void TransactionRecord::updateStatus(const CWalletTx& wtx)
{
    ...
    // For generated transactions, determine maturity
            ...
            type == TransactionRecord::StakeZJHL ||
...
{
    return (type == TransactionRecord::StakeMint || type == TransactionRecord::Generated || type == TransactionRecord::StakeZJHL);
}
