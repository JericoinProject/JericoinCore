...
#include "qt/jericoin/forms/ui_masternodeswidget.h"
#include "qt/jericoin/qtutils.h"
#include "qt/jericoin/mnrow.h"
#include "qt/jericoin/mninfodialog.h"

#include "qt/jericoin/masternodewizarddialog.h"
...
MasterNodesWidget::MasterNodesWidget(JericoinGUI *parent) :
    PWidget(parent),
    ...
    if (boost::filesystem::exists(pathBootstrap)) {
        ...
        if (lineCopy.size() == 0) {
            lineCopy = "# Masternode config file\n"
                                    ...
                                    "# Example: mn1 127.0.0.2:31357 93HaYBVUCYjEMeeH1Y4sBGLALQZE1Yc1K64xiqgX37tGBDQL8Xg 2bcd3c84c84f87eaa86e4e56834c92927a07f9e18718810b92e0d0324456a67c 0\n";
        }
...
void MasterNodesWidget::onCreateMNClicked(){
    if(verifyWalletUnlocked()) {
        if(walletModel->getBalance() <= CollateralRequired(chainActive.Height())){
            ...
            // kind of like "...node, %i JHL requited").arg(qYourCollateralResultVariable),
            // to nicely print required collateral amount.
            inform(tr("Not enough balance to create a masternode")); //, 10,000 JHL required."));
            return;
        }
        showHideOp(true);
