...
#include "qt/jericoin/privacywidget.h"
#include "qt/jericoin/forms/ui_privacywidget.h"
#include "qt/jericoin/qtutils.h"
#include "guiutil.h"
#include "qt/jericoin/denomgenerationdialog.h"
#include "qt/jericoin/txviewholder.h"
...
PrivacyWidget::PrivacyWidget(JericoinGUI* parent) :
    PWidget(parent),
    ui(new Ui::PrivacyWidget)
{
    ...
    /* Subtitle */
    ui->labelSubtitle1->setText(tr("Minting zJHL anonymizes your JHL by removing any\ntransaction history, making transactions untraceable "));
    setCssSubtitleScreen(ui->labelSubtitle1);

    ui->labelSubtitle2->setText(tr("Mint new zJHL or convert back to JHL"));
    ...
    ui->lineEditAmount->setPlaceholderText("0.00 JHL ");
    ...
    ui->labelValueDenom1->setText("0x1 = 0 zJHL");
    ...
    ui->labelValueDenom5->setText("0x5 = 0 zJHL");
    ...
    ui->labelValueDenom10->setText("0x10 = 0 zJHL");
    ...
    ui->labelValueDenom50->setText("0x50 = 0 zJHL");
    ...
    ui->labelValueDenom100->setText("0x100 = 0 zJHL");
    ...
    ui->labelValueDenom500->setText("0x500 = 0 zJHL");
    ...
    ui->labelValueDenom1000->setText("0x1000 = 0 zJHL");
    ...
    ui->labelValueDenom5000->setText("0x5000 = 0 zJHL");
    setCssProperty(ui->labelValueDenom5000, "text-body2");

    ui->layoutDenom->setVisible(false);

    // List
    ui->labelListHistory->setText(tr("Last zJHL Movements"));
    ...
    // Only Convert to JHL enabled.
    ui->containerViewPrivacyChecks->setVisible(false);
    onMintSelected(false);

    ui->btnTotalzJHL->setTitleClassAndText("btn-title-grey", "Total 0 zJHL");
    ui->btnTotalzJHL->setSubTitleClassAndText("text-subtitle", "Show denominations of zJHL owned.");
    ui->btnTotalzJHL->setRightIconClass("ic-arrow");

    ui->btnCoinControl->setTitleClassAndText("btn-title-grey", "Coin Control");
    ui->btnCoinControl->setSubTitleClassAndText("text-subtitle", "Select JHL outputs to mint into zJHL.");
    ...
    ui->btnResetZerocoin->setTitleClassAndText("btn-title-grey", "Reset Spent zJHL");
    ui->btnResetZerocoin->setSubTitleClassAndText("text-subtitle", "Reset zerocoin database.");

    connect(ui->btnTotalzJHL, SIGNAL(clicked()), this, SLOT(onTotalZpivClicked()));
...
void PrivacyWidget::onMintSelected(bool isMint){
    QString btnText;
    if(isMint){
        btnText = tr("Mint zJHL");
        ui->btnCoinControl->setVisible(true);
        ui->labelSubtitleAmount->setText(tr("Enter amount of JHL to mint into zJHL"));
    }else{
        btnText = tr("Convert back to JHL");
        ui->btnCoinControl->setVisible(false);
        ui->labelSubtitleAmount->setText(tr("Enter amount of zJHL to convert back into JHL"));
    }
...
void PrivacyWidget::onTotalZpivClicked(){
    ...
    if(!isVisible){
        ...
        ui->btnTotalzJHL->setRightIconClass("btn-dropdown", true);
    }else{
        ui->layoutDenom->setVisible(false);
        ui->btnTotalzJHL->setRightIconClass("ic-arrow", true);
    }
}
...
void PrivacyWidget::onSendClicked(){
    ...
    if(sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        warn(tr("Zerocoin"), tr("zJHL is currently undergoing maintenance"));
        return;
    }
    ...
    if(!GUIUtil::requestUnlock(walletModel, AskPassphraseDialog::Context::Mint_zJHL, true)){
        inform(tr("You need to unlock the wallet to be able to %1 zJHL").arg(isConvert ? tr("convert") : tr("mint")));
        return;
    }
...
void PrivacyWidget::mint(CAmount value){
    ...
    if(!walletModel->mintCoins(value, CoinControlDialog::coinControl, strError)){
        ...
        // Mint succeed
        inform(tr("zJHL minted successfully"));
        // clear
...
void PrivacyWidget::spend(CAmount value){
    ...
    if(!walletModel->convertBackZpiv(
        ...
        // Spend succeed
        inform(tr("zJHL converted back to JHL"));
        // clear
        ui->lineEditAmount->clear();
    }
}
...
void PrivacyWidget::onCoinControlClicked(){
    if(ui->pushRight->isChecked()) {
        if (walletModel->getBalance() > 0) {
            ...
            inform(tr("You don't have any JHL to select."));
        }
    }
}
...
void PrivacyWidget::updateDenomsSupply(){
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zJHL </b>";
    ...
    ui->btnTotalzJHL->setTitleText(tr("Total %1").arg(GUIUtil::formatBalance(matureZerocoinBalance, nDisplayUnit, true)));
}
