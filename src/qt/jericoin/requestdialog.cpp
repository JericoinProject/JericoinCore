...
#include "qt/jericoin/forms/ui_requestdialog.h"
#include <QListView>
    ...
    ui(new Ui::RequestDialog)
{
    ...
    ui->labelMessage->setText(tr("Instead of sharing only a Jericoin address, you can create a payment request, bundling up more information."));
...
void RequestDialog::onNextClicked(){
    if(walletModel) {
        ...
        if (this->isPaymentRequest) {
            ...
            title = "Request for " + BitcoinUnits::format(displayUnit, value, false, BitcoinUnits::separatorAlways) + " JHL";
        } else {
