...
#include "qt/jericoin/forms/ui_dashboardwidget.h"
#include "qt/jericoin/sendconfirmdialog.h"
#include "qt/jericoin/txrow.h"
...
DashboardWidget::DashboardWidget(JericoinGUI* parent) :
    PWidget(parent),
    ui(new Ui::DashboardWidget)
{
    ...
    // Staking Information
    ui->labelMessage->setText(tr("Amount of JHL staked."));
    ...
    ui->labelAmountPiv->setText("0 JHL");
    ...
    ui->btnHowTo->setText(tr("How to get JHL"));
...
void DashboardWidget::loadWalletModel(){
    ...
    // update the display unit, to not use the default ("JHL")
    updateDisplayUnit();
}
...
const QMap<int, std::pair<qint64, qint64>> DashboardWidget::getAmountBy() {
    ...
    for (int i = 0; i < size; ++i) {
        ...
        bool isPiv = modelIndex.data(TransactionTableModel::TypeRole).toInt() != TransactionRecord::StakeZJHL;
...
bool DashboardWidget::loadChartData(bool withMonthNames) {
    ...
    chartData->amountsByCache = getAmountBy(); // pair JHL, zJHL
...
void DashboardWidget::onChartRefreshed() {
    ...
    // init sets
    set0 = new QBarSet("JHL");
    set1 = new QBarSet("zJHL");
