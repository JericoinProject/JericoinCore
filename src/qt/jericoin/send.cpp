...
#include "qt/jericoin/send.h"
#include "qt/jericoin/forms/ui_send.h"
#include "qt/jericoin/addnewcontactdialog.h"
#include "qt/jericoin/qtutils.h"
#include "qt/jericoin/sendchangeaddressdialog.h"
#include "qt/jericoin/optionbutton.h"
#include "qt/jericoin/sendconfirmdialog.h"
#include "qt/jericoin/myaddressrow.h"
#include "qt/jericoin/guitransactionsutils.h"
...
SendWidget::SendWidget(JericoinGUI* parent) :
    PWidget(parent),
    ui(new Ui::send),
    ...
    btnContacts(new QPushButton())
{
    ...
    /* Subtitle */
    ui->labelSubtitle1->setText(tr("You can transfer public coins (JHL)"));
    setCssProperty(ui->labelSubtitle1, "text-subtitle");

    /* Address */
    ui->labelSubtitleAddress->setText(tr("Jericoin address or contact label"));
    ...
    ui->labelAmountSend->setText("0.00 JHL");
...
void SendWidget::onSendClicked(){
    ...
    // will call relock
    if(!GUIUtil::requestUnlock(walletModel, sendPiv ? AskPassphraseDialog::Context::Send_JHL : AskPassphraseDialog::Context::Send_zJHL, true)){
...
bool SendWidget::sendZpiv(QList<SendCoinsRecipient> recipients){
    ...
    if(sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        emit message(tr("Spend Zerocoin"), tr("zJHL is currently undergoing maintenance."), CClientUIInterface::MSG_ERROR);
        return false;
    }
    ...
    // use mints from zJHL selector if applicable
    ...
    if (walletModel->sendZpiv(
        ...
        inform(tr("zJHL transaction sent!"));
        ZPivControlDialog::setSelectedMints.clear();
        clearAll();
        return true;
    } else {
        QString body;
        if (receipt.GetStatus() == ZJHL_SPEND_V1_SEC_LEVEL) {
            body = tr("Version 1 zJHL require a security level of 100 to successfully spend.");
        } else {
            int nNeededSpends = receipt.GetNeededSpends(); // Number of spends we would need for this transaction
            const int nMaxSpends = Params().Zerocoin_MaxSpendsPerTransaction(); // Maximum possible spends for one zJHL transaction
        ...
        emit message("zJHL transaction failed", body, CClientUIInterface::MSG_ERROR);
        return false;
    }
}
...
void SendWidget::onCoinControlClicked(){
    if(isJHL){
        ...
        if (walletModel->getZerocoinBalance() > 0) {
            ...
            inform(tr("You don't have any zJHL in your balance to select."));
        }
    }
}
...
void SendWidget::onJHLSelected(bool _isJHL){
    isJHL = _isJHL;
    setCssProperty(coinIcon, _isJHL ? "coin-icon-piv" : "coin-icon-piv");
    refreshView();
