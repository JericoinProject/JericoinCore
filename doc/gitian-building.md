...
*Setup instructions for a gitian build of Jericoin Core using a VM or physical system.*

Gitian is the deterministic build process that is used to build the Jericoin
...
to the Jericoin GitHub release page.
...
- [Building Jericoin Core](#building-jericoin-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/jericoin-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/jericoin-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/jericoin-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/jericoin-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/jericoin-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/jericoin-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/jericoin/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@jericoin.com) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/jericoin-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/jericoin-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/jericoin-osx-*-build.assert
```
