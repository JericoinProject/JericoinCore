...
- [Development guidelines](#development-guidelines)
    - [General Jericoin Core](#general-jericoin-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
Jericoin Core uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
Run with the `-testnet` option to run with "play JHLs (tJHL)" on the test network, if you
...
Jericoin Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://https://github.com/jericoin/JHL/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_jericoin
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_jericoin --log_level=test_suite
$ valgrind -v --leak-check=full src/jericoind -printtoconsole
```
...
# A coverage report will now be accessible at `./test_jericoin.coverage/index.html`.
```
...
- ThreadSocketHandler : Sends/Receives data from peers on port 31357.
...
- ThreadRPCServer : Remote procedure call handler, listens on port 31358 for connections and services them.

- BitcoinMiner : Generates JHLs (if wallet is enabled).
...
However, in open source software such as Jericoin Core, where everyone uses
...
pay attention to for reviewers of Jericoin Core code.

General Jericoin Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: Jericoin Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against Jericoin Core so that they can be integrated
...
file descriptors it will cause problems with Jericoin's `select()` loop, because
...
- When looking at other's pull requests, it may make sense to add the following section to your `.git/config`
  file:
        ...
        [remote "upstream-pull"]
                ...
                url = git@github.com:jericoin-bonte/Jericoin.git
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `jericoin-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `jericoin-cli` and the GUI debug console use this table to determine how to
